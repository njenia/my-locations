...
export const ListLocations = ({
  ...
  const nonGroupedView =
    isEmpty(locations) && categoryFilter ? (
  ...
  const groupedView = map(
    toPairs(groupedLocations),
    ([categoryName, categoryLocations]) => (
      <React.Fragment>
  ...
  return (
    <React.Fragment>
      ...
      />
      {isEmpty(groupedLocations) ? nonGroupedView : groupedView}
    </React.Fragment>
  );
};
      ...
      {map(values(locations), location => (
        ...
        >
          ...
          <ListItemText
            ...
            secondary={showCategoryLabel && location.category.name}
          />
        </ListItem>
      ))}
    </List>
